...
const createEnquiry = (req, res, enquiry, prm_client_id) => {
  ...
  if (enquiry.prm_dentist_user_id)
    statement += enquiry.prm_dentist_user_id + ','
  if (enquiry.gender) statement += "'" + enquiry.gender + "',"
  if (enquiry.lastName) statement += "'" + enquiry.lastName + "',"
  if (enquiry.lead_owner_id) {
    statement += enquiry.lead_owner_id + ','
  } else {
    statement += '0,'
  }
  statement += 'NOW(),NOW(),'
...
const updateEnquiry = (req, res, id, enquiry) => {
  pool.query(
    'SELECT * FROM enquiries WHERE enquiries.id = $1',
    [id],
    (error, results) => {
      ...
      if (enquiry.name !== currentEnquiry.name)
        statement += "name='" + enquiry.name + "',"
      if (enquiry.last_name !== currentEnquiry.last_name)
        statement += "last_name='" + enquiry.last_name + "',"
      if (
        enquiry.date_of_birth &&
        enquiry.date_of_birth !==
          moment(currentEnquiry.date_of_birth).format('YYYY-MM-DD')
      )
        statement += "date_of_birth='" + enquiry.date_of_birth + "',"
      if (enquiry.gender !== currentEnquiry.gender)
        statement += "gender='" + enquiry.gender + "',"
      if (enquiry.address_line_1 !== currentEnquiry.address_line_1)
        statement += "address_line_1='" + enquiry.address_line_1 + "',"
      if (enquiry.post_code !== currentEnquiry.post_code)
        statement += "post_code='" + enquiry.post_code + "',"
      if (enquiry.city !== currentEnquiry.city)
        statement += "city='" + enquiry.city + "',"
      if (enquiry.country_id !== currentEnquiry.country_id)
        statement += "country_id='" + enquiry.country_id + "',"
      if (enquiry.region_id !== currentEnquiry.region_id)
        statement += "region_id='" + enquiry.region_id + "', "
      if (enquiry.phone !== currentEnquiry.phone)
        statement += "phone='" + enquiry.phone + "', "
      if (enquiry.email !== currentEnquiry.email)
        statement += "email='" + enquiry.email + "',"
      if (enquiry.prm_dentist_user_id !== currentEnquiry.prm_dentist_user_id)
        statement +=
          "prm_dentist_user_id='" + enquiry.prm_dentist_user_id + "',"
      if (
        enquiry.tax_registration_number !==
        currentEnquiry.tax_registration_number
      )
        statement +=
          "tax_registration_number='" + enquiry.tax_registration_number + "',"
      if (enquiry.insurance_no !== currentEnquiry.insurance_no)
        statement += "insurance_no='" + enquiry.insurance_no + "',"
      if (enquiry.insured_at !== currentEnquiry.insured_at)
        statement += "insured_at='" + enquiry.insured_at + "',"
      ...
      if (enquiry.prm_surgeon_user_id !== currentEnquiry.prm_surgeon_user_id)
        statement +=
          "prm_surgeon_user_id='" + enquiry.prm_surgeon_user_id + "',"
...
const trashEnquiry = (request, response, id) => {
  pool.query(
    'UPDATE enquiries SET trashed = true WHERE id = $1',
    [id],
    (error, results) => {
      if (error) {
        throw error
...
const getEnquiryNotes = (request, response, enquiryId) => {
  let statement =
    "SELECT content, notes.created_at, user_id, concat(users.title, ' ', users.first_name , ' ', users.surname) AS user_name FROM notes "
...
const getEnquiryPastAppointments = (request, response, enquiryId, locale) => {
  pool.query(
    'SELECT appointments.*, enquiries.*, prm_product_group_name.*, appointments_label.*, ' +
      ...
      "WHERE enquiry_id = $1 AND prm_product_group_name.language = '" +
      locale +
      "' AND starts_at < now()::date ORDER BY starts_at DESC",
    [enquiryId],
    (error, results) => {
      if (error) {
        throw error
...
const getEnquiryFutureAppointments = (request, response, enquiryId, locale) => {
  pool.query(
    'SELECT appointments.*, enquiries.*, prm_product_group_name.*, appointments_label.*, ' +
      ...
      "WHERE enquiry_id = $1 AND prm_product_group_name.language = '" +
      locale +
      "' AND starts_at >= now()::date ORDER BY starts_at ASC",
    [enquiryId],
    (error, results) => {
      if (error) {
        throw error
...
const getEnquiryAssignments = (request, response, enquiryId) => {
  let statement = [
    "SELECT enquiries.id AS id, todos.description AS description, todos.completed AS completed, todos.due_at AS due_at, concat(title, ' ', first_name, ' ', surname) AS name FROM todos ",
...
const getEnquiryOffers = (request, response, enquiryId) => {
  pool.query(
    "SELECT * FROM invoice WHERE invoice_type = 'Offer' AND enquiries_id = $1",
    [enquiryId],
    (error, results) => {
      if (error) {
        throw error
...
const getEnquiryServices = (request, response, enquiryId) => {
  pool.query(
    'SELECT *, products.name as product_name, services.created_at AS done_at, u.name as doctor FROM services LEFT JOIN products ON services.product_id = products.id LEFT JOIN client_users cu ON services.doctor_id = cu.id LEFT JOIN users u ON cu.user_id = u.id WHERE enquiry_id = $1 ORDER BY done_at DESC',
    [enquiryId],
    (error, results) => {
      if (error) {
        throw error
...
const createEnquiryService = (req, res, enquiryId, service) => {
  var statement =
    "INSERT INTO services (title, product_id, price, date, created_at, payment_method, doctor_id, enquiry_id, fee) VALUES ('" +
    service.title +
    "', " +
    service.product_id +
    ', ' +
