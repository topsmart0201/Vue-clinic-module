...
const getApontments = (
  ...
  statement +=
    'LEFT JOIN appointment_slots app_s ON app.id = app_s.appointment_id '
  ...
  statement +=
    'LEFT JOIN appointments_label app_lb ON app.label_id = app_lb.id '
  statement +=
    'LEFT JOIN prm_product_group prm_pr_group ON app.product_group_id = prm_pr_group.product_group_id '
  statement +=
...
const updateAppointments = (request, response, id, appointments) => {
  ...
  if (appointments.location)
    statement += "location='" + appointments.location + "',"
  if (appointments.notes) statement += "note='" + appointments.notes + "',"
  if (appointments.patient_id)
    statement += 'enquiry_id=' + appointments.patient_id + ','
  if (appointments.patient_attended)
    statement += "patient_attended='" + appointments.patient_attended + "',"
  if (appointments.appointment_canceled)
    statement +=
  ...
  if (appointments.assignmentDate)
    statement +=
      "starts_at='" +
      moment(appointments.assignmentDate).format('YYYY-MM-DDTHH:mm') +
      "', date='" +
      moment(appointments.assignmentDate).format('YYYY-MM-DD') +
      "',"
  if (appointments.backgroundColor)
    statement += "label_id='" + appointments.backgroundColor + "',"
  if (appointments.end)
    statement +=
      "ends_at='" +
      moment(appointments.assignmentDate).format('YYYY-MM-DD') +
      'T' +
      moment(appointments.end).format('HH:mm') +
      "',"
  if (appointments.assignmentDate) statement += "time='" + time + "',"
  if (appointments.patient_attended === true)
    statement += "attendance='Attended',"
...
const createAppointment = (request, response, appointments) => {
  ...
  if (appointments.doctor_name)
    statement += "'" + appointments.doctor_name + "',"
  ...
  if (appointments.patient_attended)
    statement += "'" + appointments.patient_attended + "',"
  if (appointments.crmProduct) statement += '' + appointments.crmProduct + ','
  if (appointments.product_groups)
    statement += '' + appointments.product_groups + ','
  if (appointments.assignmentDate)
    statement +=
      "'" +
      moment(appointments.assignmentDate).format('YYYY-MM-DDTHH:mm') +
  ...
  pool.query(statement, (error, results) => {
    ...
    pool.query(
      'SELECT * FROM appointments WHERE id=(SELECT max(id) FROM appointments)',
      (err, res) => {
...
const updateAppointmentsLabel = (request, response, id, appointmentsLabel) => {
  pool.query(
    'SELECT * FROM appointments_label WHERE id = $1',
    [id],
    (error, results) => {
      ...
      if (appointmentsLabel.type)
        statement += "type='" + appointmentsLabel.type + "' "
      statement = statement.slice(0, -1)
      ...
      pool.query(statement, (error, results) => {
        ...
        updateAppointmentsLabelName(
          appointmentsLabel.id,
          appointmentsLabel.lang,
