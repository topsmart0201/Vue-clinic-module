<template>
  <b-container fluid>
    <div style="display: none">
      <div
        id="printInvoice"
        style="color: black; margin-left: 40px; height: 1200px"
        v-if="invoice"
      >
        <b-row>
          <b-col lg="12">
            ...
            <p>
              {{ $t('advPayments.newAdvPayment.IBAN') }}:
              {{ invoice.company_iban }}
            </p>
            <p>
              {{ $t('advPayments.newAdvPayment.vatId') }}:
              {{ invoice.company_tax_registration_number }}
            </p>
            <p>
              {{ $t('advPayments.newAdvPayment.regNumber') }}:
              {{ invoice.company_legal_registration_identifier }}
            </p>
          </b-col>
        </b-row>
        <b-row>
          <b-col lg="6">
            <p>
              {{ invoice.enquiries_name }} {{ invoice.enquiries_last_name }}
            </p>
            <p>{{ invoice.enquiries_address_line_1 }}</p>
            <p>
              {{ invoice.enquiries_post_code }} {{ invoice.enquiries_city }}
            </p>
          </b-col>
          <b-col lg="6">
            <p>
              {{ $t('invoices.newInvoice.invoiceNo') }}:
              {{ invoice.reference_code_furs }}
            </p>
            <p>
              {{ $t('advPayments.newAdvPayment.copy') }}:<span
                style="margin-left: 20px"
                >Original</span
              >
            </p>
            <p>
              {{ $t('advPayments.newAdvPayment.IssuedIn') }}:<span
                style="margin-left: 20px"
                >{{ premiseCity }}</span
              >
            </p>
            <p>
              {{ $t('advPayments.newAdvPayment.dateOfAdvPayment') }}:<span
                style="margin-left: 20px"
                >{{ invoiceTime }}</span
              >
            </p>
        ...
        <b-row>
          <b-table-simple small responsive>
            <b-thead>
              <b-tr>
                <b-th colspan="2">{{
                  $t('invoice.invoiceInfo.invoiceDate')
                }}</b-th>
                <b-th colspan="2">{{
                  $t('advPayments.newAdvPayment.code')
                }}</b-th>
                <b-th colspan="2">{{
                  $t('advPayments.newAdvPayment.item')
                }}</b-th>
                <b-th colspan="1">{{
                  $t('advPayments.newAdvPayment.quantity')
                }}</b-th>
                <b-th colspan="1">{{
                  $t('advPayments.newAdvPayment.price')
                }}</b-th>
                <b-th colspan="2">{{
                  $t('advPayments.newAdvPayment.value')
                }}</b-th>
                <b-th colspan="2">{{
                  $t('advPayments.newAdvPayment.amount')
                }}</b-th>
              </b-tr>
            </b-thead>
            <b-tbody>
              ...
              <b-tr>
                ...
                <b-td colspan="2"
                  ><strong
                    >{{ $t('advPayments.newAdvPayment.total') }}:
                  </strong></b-td
                >
              ...
              <b-tr>
                ...
                <b-td colspan="2"
                  ><strong
                    >{{ $t('advPayments.newAdvPayment.payment') }}:
                  </strong></b-td
                >
                <b-td colspan="2">{{ invoice.lines_sum | euro }}</b-td>
              </b-tr>
        ...
        <b-row>
          ...
          <b-col lg="6">
            <p>
              {{ $t('advPayments.newAdvPayment.issuedBy') }}:
          ...
          <template v-slot:body>
            <b-row>
              <b-col cols="4" align-self="center" style="margin-bottom: 25px">
                <h4 class="mb-0">
                  {{ $t('invoice.invoiceHeader') }} N° : {{ invoiceHeader }}
                </h4>
              </b-col>
            </b-row>
            <b-row>
                  ...
                  <b-table :items="invoices" :fields="invoiceColumns">
                    <template v-slot:cell(payment_status)="data">
                      <span
                        ...
                        v-if="data.value == 'Unpaid'"
                        >{{ $t('invoice.unpaid') }}</span
                      >
                      <span
                        class="badge badge-warning"
                        v-if="data.value == 'Partialy Paid'"
                        >{{ $t('invoice.partialyPaid') }}</span
                      >
                      <span
              ...
              <b-col lg="2">
                <div class="table-responsive-sm">
                  <b-table-simple striped>
                    <b-thead>
                      <b-th colspan="2">{{
                        $t('advPayments.advPaymentsColumn.date')
                      }}</b-th>
                    </b-thead>
                    <b-tbody>
                      <b-tr>
                ...
                <div class="table-responsive-sm">
                  ...
                  >
                    ...
                    <template v-slot:cell(teeth)="data">
                      <v-select
                        class="style-chooser"
                        v-if="Array.isArray(data.item.teeth)"
                        disabled
                        multiple
                        v-model="data.item.teeth"
                    ...
                    <template v-slot:cell(surfaces)="data">
                      <v-select
                        ...
                        disabled
                        multiple
                        v-model="data.item.surfaces"
                        :options="surfaceOptions"
                      ></v-select>
                    </template>
                    <template v-slot:cell(product_price)="data">
                      <span>{{
                        data.item.product_price | numeral('0,0.00') | euro
                      }}</span>
                    </template>
                    ...
                    <template v-slot:cell(net_amount)="data">
                      <span>{{
                        data.item.net_amount | numeral('0,0.00') | euro
                      }}</span>
                    </template>
                  </b-table>
                </div>
            ...
            <b-row>
              <b-col class="mt-4" lg="6" offset-lg="6">
                <h2 class="text-center">
                  {{ $t('advPayment.advPaymentInfo.advPaymentTotal') }}:
                  {{ itemAmount | euro }}
                </h2>
              </b-col>
            </b-row>
            <b-row>
              <b-col class="mt-4" lg="8">
                <h5 style="margin-bottom: 15px">
                  {{ $t('advPayments.newAdvPayment.paymentMethodSummary') }}
                </h5>
                <div class="table-responsive-sm">
                  <b-table
                    striped
                  ...
                  >
                    <template v-slot:cell(amount)="data">
                      <span class="font-weight-bold">{{
                        data.item.amount | numeral('0,0.00') | euro
                      }}</span>
                    </template>
                  </b-table>
                </div>
...
import {
  getPremiseById,
  getPremiseDeviceById,
} from '../../services/companyPremises'
...
export default {
  ...
  computed: {
    invoiceHeader() {
      return this.invoiceNumber.includes('.')
        ? this.$t('invoice.draft')
        : this.invoiceNumber
    },
  },
  methods: {
    getInvoice(id) {
      getInvoiceById(id).then((response) => {
        ...
        this.invoiceDatePdf = moment(this.invoice.invoice_time).format(
          'DD.MM.YYYY HH:MM',
        )
        this.invoiceDate = moment(this.invoice.invoice_time).format(
          'DD.MM.YYYY',
        )
    ...
    calculateQRCode() {
      ...
      this.decimalZoi =
        decimalNumber.length < 39
          ? '0'.repeat(39 - decimalNumber.length) + decimalNumber
          : decimalNumber
      let timeStamp = moment(this.invoice.invoice_time).format('DDMMYYHHMMSS')
      this.qrCode =
        this.decimalZoi + this.invoice.operator_tax_number + timeStamp
    ...
    exportToPDF() {
      ...
      this.invoiceTime = moment(this.invoice.invoice_time).format(
        'DD.MM.YYYY HH:MM',
      )
      let options = {
    ...
    calculatePdfNumber() {
      let premiseNumber =
        this.premiseId < 10 ? '0' + this.premiseId : this.premiseId
      this.pdfNumber =
        premiseNumber +
        '-' +
        this.deviceName +
      ...
      invoiceColumns: [
        ...
        },
        {
          ...
          formatter: (value, key, item) => {
            ...
            if (item.enquiries_last_name) {
              details += ' ' + item.enquiries_last_name
            }
            details += '<br>'
            if (item.enquiries_address_line_1) {
              details += item.enquiries_address_line_1 + '<br>'
            }
        ...
        {
          label: this.$t('invoice.invoiceInfo.invoiceIssuedIn'),
          key: 'company_city',
          class: 'text-left',
        },
        {
          label: this.$t('invoice.invoiceInfo.invoiceIssuedBy'),
          key: 'operator_name',
          class: 'text-left',
        },
      ],
